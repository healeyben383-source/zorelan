...
const fields = [
  [
    "verified_answer",
    "The synthesized final answer combining the best of both models.",
  ],
  ["confidence", "high / medium / low — how much the models agreed."],
  [
    "confidence_reason",
    "A plain English explanation of the confidence level.",
  ],
  ...
  [
    "meta.task_type",
    "The detected category of your query (strategy, technical, creative, general).",
  ],
  ["meta.overlap_ratio", "0.0–1.0 score of how similar the two model outputs were."],
  [
    "meta.agreement_summary",
    "A one-line summary of how the models compared.",
  ],
  [
    "usage",
    "Returned for paid API keys. Includes plan, monthly limit, used calls and remaining calls.",
  ],
] as const;
    ...
    <main className="min-h-screen bg-black text-white px-6 py-16 max-w-3xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <p className="text-white/60 text-lg leading-relaxed">
          Get a verified, multi-model AI answer in a single API call. Zorelan
          runs multiple AI models, compares their outputs, and returns a
          synthesized answer with a confidence score.
        </p>
      </div>
      ...
      <section className="mb-12">
        ...
        <div className="rounded-2xl border border-white/10 p-6 space-y-4">
          <p className="text-white/80">
            Start a sandbox subscription below to test the Zorelan API checkout
            flow.
          </p>
          <PricingButtons />
        </div>
      ...
      <section className="mb-12">
        ...
        <div className="rounded-2xl border border-white/10 p-6">
          <div className="flex items-center gap-3">
            <span className="text-xs font-mono bg-white/10 text-white px-2 py-1 rounded">
              POST
            </span>
        ...
        <div className="rounded-2xl border border-white/10 divide-y divide-white/10">
          {fields.map(([field, desc]) => (
            <div key={field} className="flex gap-4 px-6 py-4 text-sm">
              <span className="font-mono text-white/60 w-48 shrink-0">
                {field}
              </span>
      ...
      <section>
        <div className="rounded-2xl border border-white/10 p-8 text-center space-y-4">
          ...
          <p className="text-white/50">
            Choose a plan and continue through Stripe Checkout to test the
            billing flow.
          </p>
          <PricingButtons />
        </div>
